...
export const init = new Command()
    .name("init")
    .description("Initialize component library and global styles automatically")
    .action(async () => {
        const spinner = ora("Initializing curv-ui...").start();

        try {
            const cwd = process.cwd();

            // Try to locate a global CSS file
            const cssPaths = [
            ...
            if (!cssPath) {
                spinner.fail(
                    "❌ Could not find a global CSS file (e.g., globals.css or index.css)."
                );
                console.log(
                    chalk.yellow(
                        "Please create one manually, then re-run `npx curv-ui init`."
                    )
                );
                return;
            }
...
export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
            ...
            spinner.fail("❌ Failed to initialize theme.");
            console.error(error);
        }
    });
